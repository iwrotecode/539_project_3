...
class Database {
	...
	function doQuery($query, $vars = array(), $types = array()) {
		...
		if (substr_count($query, "?") != count($vars) || count($vars) != count($types)) {
			...
			if ($select) {
				...
				$bind_res[0] = $stmt;
				//make the statement first element
	...
	function getColInfo($tableName) {
		...
		if ($cols) {
			while ($col = $cols -> fetch_assoc()) {
				$cols_return[$col['Field']] = array("Type" => $col['Type'], "Null" => $col['Null'], "Key" => $col['Key'], "Default" => $col['Default'], "Extra" => $col['Extra']);
			}
		} else {
			$this -> error = $this -> mysqli -> error;
		}
	...
	private function get_type_name($code, $size, $decimals, $charset, $flags) {
		switch ($code) {
			...
			case 14 :
				return "NEWDATE";
			// according to http://www.redferni.uklinux.net/mysql/MySQL-Protocol.html
			case 16 :
				return "BIT($size)";
			case 246 :
			...
			// according to http://www.redferni.uklinux.net/mysql/MySQL-Protocol.html
			case 252 :
				if ($charset == 63) {// 63 is binary pseudocollation, used for non-string types
					if ($size == 255)
						return 'TINYBLOB / TINYTEXT BINARY';
					if ($size == 65535)
						return 'BLOB / TEXT BINARY';
					if ($size == 16777215)
						return 'MEDIUMBLOB / MEDIUMTEXT BINARY';
					if ($size == -1)
						return 'LONGBLOB / LONGTEXT BINARY';
				} else {
					if ($size == 255)
						return 'TINYTEXT';
					if ($size == 65535)
